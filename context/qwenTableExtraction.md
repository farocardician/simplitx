# Phase 1 — Implementation Plan (Header & Column Bands)

**Goal**
From `s02-table-pymupdf-tokens.json`, deterministically identify the correct header row and produce precise **x-bands** for each field. No models used in this phase.
...
* `--in`: path to `s02-table-pymupdf-tokens.json` (PyMuPDF tokens with page, text, bbox, order).
* `--vendor`: vendor/config key (to load synonyms and required fields).
...
* `--overlay`: write `s05_bands_overlay.svg` per page with vertical band lines and the chosen header highlighted. This makes QA trivial.
...
* Check `bands_order` is exactly `["no","description","qty","uom","unit_price","amount"]` (or your vendor schema).
...
* Keep all thresholds in the vendor config (`y_tolerance_header`, `x_margin_ratio`, min coverage).
