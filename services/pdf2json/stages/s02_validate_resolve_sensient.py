...
- Resolve buyer_party_id from buyer_name using parties in the database (exact + fuzzy).
...
REQUIRED_FIELDS = ['invoice', 'buyer_name', 'input_quantity', 'unit_price', 'amount']
...
def process_batch(conn, batch_id: str):
    ...
    for _, row in df.iterrows():
        ...
        party_id, matched_name, confidence = resolve_buyer_party_id(row.get('buyer_name'), parties_dict)
