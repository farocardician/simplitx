...
class ColumnConfig:
    ...
    name: str
    synonyms: Sequence[str]  # Header text variations
    normalizer: Optional[str] = None  # e.g., "currency", "integer", "string"
    output_fields: Optional[Sequence[str]] = None  # For multi-output columns (raw + normalized)
    truncate_at: Optional[int] = None  # Max length for string fields
...
class RegionConfig:
    ...
    columns: Optional[List[ColumnConfig]] = None  # For table regions
    use_reranker: bool = True  # Whether to use reranker for this region
...
def _parse_region_data(region_name: str, region_data: dict, table_prefix: str = "rag") -> RegionConfig:
    ...
    use_reranker = bool(region_data.get("use_reranker", True))

    # Parse columns for table regions
    columns: Optional[List[ColumnConfig]] = None
    columns_raw = region_data.get("columns")
    if isinstance(columns_raw, list):
        ...
        for col_data in columns_raw:
            ...
            if not col_name:
                continue
            col_synonyms = col_data.get("synonyms", [])
            if isinstance(col_synonyms, str):
                col_synonyms = [col_synonyms]
            col_normalizer = col_data.get("normalizer")
            col_output_fields = col_data.get("output_fields")
            col_truncate_at = col_data.get("truncate_at")
            columns.append(
                ColumnConfig(
                    name=col_name,
                    synonyms=col_synonyms,
                    normalizer=col_normalizer,
                    output_fields=col_output_fields,
                    truncate_at=col_truncate_at,
                )
            )
...
def resolve_tokenizer_sources(tokenizer_arg: Optional[str]) -> Optional[set]:
    """Resolve --tokenizer CLI argument to canonical source names.

    Returns None if no filtering (use all sources as before).
    Returns a set of source names if filtering is requested.
    """
    if tokenizer_arg is None:
        return None  # No filtering, use all sources

    tokenizer_arg = tokenizer_arg.strip()
    if not tokenizer_arg:
        raise ValueError("--tokenizer cannot be empty. Omit the flag to use all sources.")

    # Aliases for convenience
    ALIASES = {
        "plumber": "plumber.tokens",
        "pymupdf": "pymupdf.tokens",
        "ocr": "ocr.tokens",
    }

    parts = [p.strip() for p in tokenizer_arg.split(",") if p.strip()]
    if not parts:
        raise ValueError("--tokenizer must specify at least one source")
    ...
    for part in parts:
        if part in ALIASES:
            resolved.add(ALIASES[part])
        else:
            # Could be explicit like "plumber.tokens", "plumber.words", "plumber.pages", "pages", "tokens"
            resolved.add(part)

    return resolved


def extract_tokens_page1(s02: dict, source_filter: Optional[set] = None) -> Tuple[List[Token], Dict[str, int]]:
    """Extract normalized tokens belonging to page 1.

    Args:
        s02: S02 JSON document
        source_filter: Optional set of source names to use. None means use all sources.

    Returns:
        Tuple of (tokens, source_counts) where source_counts tracks how many tokens from each source.
    """
    ...
    def should_use_source(source_name: str) -> bool:
        """Check if this source should be used based on filter."""
        if source_filter is None:
            return True
        return source_name in source_filter

    def add_tokens(raw_tokens, page_hint: Optional[int], source_name: str = "unknown") -> None:
        if not should_use_source(source_name):
            return
        ...
        if tokens_added > 0:
            source_counts[source_name] = source_counts.get(source_name, 0) + tokens_added

    if isinstance(s02, dict):
        # Top-level pages array
        ...
        if isinstance(pages, list):
            for idx, page_entry in enumerate(pages):
                ...
                if isinstance(page_entry, dict):
                    ...
                    add_tokens(page_entry.get("tokens") or page_entry.get("items"), page_hint, "pages")

        # Top-level tokens array
        add_tokens(s02.get("tokens"), None, "tokens")

        # Structured providers: plumber, pymupdf, ocr
        for provider_name in ("plumber", "pymupdf", "ocr"):
            container = s02.get(provider_name)
            if isinstance(container, dict):
                add_tokens(container.get("tokens"), None, f"{provider_name}.tokens")
                add_tokens(container.get("words"), None, f"{provider_name}.words")
                    ...
                    for idx, page_entry in enumerate(pages_like):
                        if isinstance(page_entry, dict):
                            ...
                            add_tokens(
                                ...
                                page_hint,
                                f"{provider_name}.pages",
                            )
    elif isinstance(s02, list):
        add_tokens(s02, None, "root")

    if not tokens:
        if source_filter:
            requested = ", ".join(sorted(source_filter))
            raise RuntimeError(
                f"No tokens available from requested sources: {requested}. "
                "Check --tokenizer or your S02 input."
            )
        else:
...
def _normalize_integer(text: str) -> str:
    ...
    Extracts the first integer from the text.
...
def _apply_normalizer(text: str, normalizer: Optional[str]) -> str:
    ...
    elif normalizer_lower == "integer":
        return _normalize_integer(text)
    elif normalizer_lower == "string":
        return text.strip()  # String normalizer just strips whitespace
...
class ColumnPosition:
    """Represents a detected table column with its boundaries."""
    name: str
    x_min: float
    x_max: float
    config: ColumnConfig
...
def detect_table_header(
    ...
    echo: bool = False,
) -> Optional[Tuple[List[ColumnPosition], float]]:
    """Detect table header row and determine column boundaries.

    Args:
        tokens: All tokens in the table region
        columns_config: Column configurations from config
        echo: Enable debug logging

    Returns:
        Tuple of (column_positions, header_y_max) or None if header not detected
    """
    if not tokens or not columns_config:
        return None

    # Group tokens by approximate y-position (rows)
    rows: Dict[float, List[Token]] = {}
    for token in tokens:
        y_center = (token["bbox"][1] + token["bbox"][3]) / 2.0
    ...
    # Try to find header row by matching column synonyms
    header_row_tokens: Optional[List[Token]] = None
    header_y: Optional[float] = None

    for y_key, row_tokens in sorted_rows[:5]:  # Check first 5 rows
        row_text = " ".join(t["text"] for t in sorted(row_tokens, key=lambda t: t["bbox"][0]))
        row_text_lower = row_text.lower()

        # Count how many column headers we can find
        matches = 0
        for col_cfg in columns_config:
            for synonym in col_cfg.synonyms:
                if synonym.lower() in row_text_lower:
                    matches += 1
                    break

        # If we match at least 50% of columns, consider this the header row
        if matches >= len(columns_config) * 0.5:
            header_row_tokens = row_tokens
            header_y = y_key
            if echo:
                logger.info("Detected table header at y=%.3f: %s", y_key, row_text[:80])
            break

    if not header_row_tokens or header_y is None:
        if echo:
            logger.warning("Could not detect table header row")
        return None

    # Determine column boundaries based on header token positions
    column_positions: List[ColumnPosition] = []
    matched_tokens: set = set()

    for col_cfg in columns_config:
        # Find tokens that match this column's synonyms
        matching_tokens: List[Token] = []
        for token in header_row_tokens:
            if id(token) in matched_tokens:
                continue
            token_text = token["text"].lower()
            for synonym in col_cfg.synonyms:
                if synonym.lower() in token_text or token_text in synonym.lower():
                    matching_tokens.append(token)
                    matched_tokens.add(id(token))
                    break

        if matching_tokens:
            # Use the bounding box of matching tokens to define column x-range
            x_min = min(t["bbox"][0] for t in matching_tokens)
            x_max = max(t["bbox"][2] for t in matching_tokens)
            # Expand a bit to catch nearby values
            x_range = x_max - x_min
            x_min -= x_range * 0.3
            x_max += x_range * 0.3
            column_positions.append(ColumnPosition(col_cfg.name, x_min, x_max, col_cfg))

    if not column_positions:
        if echo:
            logger.warning("Could not match any column headers")
        return None

    # Determine header row's bottom y-coordinate
    header_y_max = max(t["bbox"][3] for t in header_row_tokens)

    if echo:
        logger.info("Detected %d columns:", len(column_positions))
        for col_pos in column_positions:
            logger.info("  %s: x=[%.3f, %.3f]", col_pos.name, col_pos.x_min, col_pos.x_max)

    return column_positions, header_y_max
...
def extract_table_rows(
    ...
    """Extract table data rows based on column boundaries.

    Args:
        tokens: All tokens in the table region
        column_positions: Detected column boundaries
        header_y_max: Y-coordinate below which to look for data rows
        echo: Enable debug logging

    Returns:
        List of row dictionaries, each with column names as keys
    """
    if not tokens or not column_positions:
        return []

    # Filter tokens that are below the header
    data_tokens = [t for t in tokens if t["bbox"][1] >= header_y_max]
    ...
    for y_key, row_tokens in sorted_rows:
        row_data: Dict[str, List[Token]] = {col_pos.name: [] for col_pos in column_positions}

        # Assign each token to a column based on x-position
        for token in row_tokens:
            token_x_center = (token["bbox"][0] + token["bbox"][2]) / 2.0
            ...
            for col_pos in column_positions:
                # Check if token center is within column range
                if col_pos.x_min <= token_x_center <= col_pos.x_max:
                    overlap = min(token["bbox"][2], col_pos.x_max) - max(token["bbox"][0], col_pos.x_min)
                    if overlap > best_overlap:
                        best_overlap = overlap
                        best_col = col_pos.name

            if best_col:
                row_data[best_col].append(token)

        # Build cell text for each column
        row_dict: Dict[str, str] = {}
        has_any_data = False

        for col_pos in column_positions:
            cell_tokens = sorted(row_data[col_pos.name], key=lambda t: t["bbox"][0])
            cell_text = _join_tokens(cell_tokens)

            # Apply normalizer if configured
            if col_pos.config.normalizer:
                normalized = _apply_normalizer(cell_text, col_pos.config.normalizer)
            else:
                normalized = cell_text.strip()

            # Apply truncation if configured
            if col_pos.config.truncate_at and len(cell_text) > col_pos.config.truncate_at:
                cell_text = cell_text[:col_pos.config.truncate_at]

            # Handle multi-output columns (e.g., raw + normalized)
            if col_pos.config.output_fields:
                for field_name in col_pos.config.output_fields:
                    if field_name.endswith("_norm"):
                        row_dict[field_name] = normalized if normalized else None
                    else:
                        row_dict[field_name] = cell_text if cell_text else None
                    if cell_text or normalized:
                        has_any_data = True
            else:
                row_dict[col_pos.name] = normalized if normalized else None
                if normalized:
                    has_any_data = True
    ...
    if echo:
        logger.info("Extracted %d data rows", len(table_rows))
        if table_rows:
            logger.info("First row: %s", table_rows[0])
            if len(table_rows) > 1:
                logger.info("Last row: %s", table_rows[-1])
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument(
        "--tokenizer",
        default=None,
        help="Comma-separated list of token sources to use (e.g., 'plumber,pymupdf' or 'plumber.tokens,ocr.words'). "
             "Omit to use all available sources (default). Aliases: plumber→plumber.tokens, pymupdf→pymupdf.tokens, ocr→ocr.tokens",
    )
    ...
    try:
        ...
        # Resolve tokenizer sources
        source_filter = resolve_tokenizer_sources(args.tokenizer)
        if args.echo:
            if source_filter is None:
                logger.info("Token sources: using all available sources (default)")
            else:
                logger.info("Token sources: %s", ", ".join(sorted(source_filter)))

        s03 = load_json(args.s03)
        s02 = load_json(args.s02)
        tokens, source_counts = extract_tokens_page1(s02, source_filter)

        if args.echo and source_counts:
            logger.info("Token counts by source:")
            for source_name in sorted(source_counts.keys()):
                logger.info("  %s: %d tokens", source_name, source_counts[source_name])
        ...
        for region_name, region_config in rag_config.regions.items():
            ...
            if region_bbox is None:
                logger.warning("Region '%s' not found in S03; skipping.", region_name)
                # For table regions, set line_items to empty array
                if region_config.columns:
                    line_items = []
                else:
                    # For non-table regions, set all fields to empty string (null in JSON)
                    for field_name in region_config.query_synonyms.keys():
                        all_answers[field_name] = ""
                continue

            # Handle table regions differently
            if region_config.columns:
                if args.echo:
                    logger.info("Table region detected with %d columns", len(region_config.columns))

                # Filter tokens in this region bbox
                region_tokens = [t for t in tokens if intersects(t["bbox"], region_bbox)]

                if not region_tokens:
                    logger.warning("No tokens detected within %s bbox.", region_name)
                    line_items = []
                    continue

                # Detect table header and extract rows
                header_result = detect_table_header(region_tokens, region_config.columns, echo=args.echo)
                if header_result is None:
                    logger.warning("Could not detect table header in %s.", region_name)
                    line_items = []
                    continue

                column_positions, header_y_max = header_result
                line_items = extract_table_rows(region_tokens, column_positions, header_y_max, echo=args.echo)

                if args.echo:
                    logger.info("Extracted %d line items from table", len(line_items))

            else:
                # Non-table region: use RAG pipeline
                ...
                # Ensure schema for this region's table (only if use_reranker is true)
                if region_config.use_reranker:
                    table_name = region_config.table_name
                    ...
                    # Skip RAG for regions with use_reranker=false
                    if args.echo:
                        logger.info("Skipping RAG indexing/queries for %s (use_reranker=false)", region_name)

        # Merge outputs
        output = dict(all_answers)
        ...
        if args.echo:
            logger.info("Final output: %d flat fields, %d line items", len(all_answers), len(line_items))
