"""Header-only RAG pipeline for invoice metadata extraction.

This module rebuilds reading-order lines from S02 tokens, filters header
segments based on S03, indexes them into Postgres/pgvector, and performs
...
DEFAULT_CONFIG_PATH = "services/pdf2json/config/s04_invoice_esi_rag_v1.json"
...
def load_rag_config(path: Optional[str]) -> RagConfig:
    ...
    query_synonyms = data.get("query_synonyms") or {}
    if not isinstance(query_synonyms, dict):
        raise RuntimeError("RAG config 'query_synonyms' must be an object mapping fields.")

    fields_raw = data.get("fields") or {}
    if not isinstance(fields_raw, dict):
        raise RuntimeError("RAG config 'fields' must be an object mapping field configs.")
    ...
    for field_name in query_synonyms.keys():
        if field_name not in fields:
            fields[field_name] = FieldConfig(name=field_name)

    # Cast query synonyms values to sequences
    normalized_synonyms: Dict[str, Sequence[str]] = {}
    for field_name, synonyms in query_synonyms.items():
    ...
    return RagConfig(query_synonyms=normalized_synonyms, fields=fields)
...
def find_header_bbox(s03: dict, page: int = 1) -> Tuple[float, float, float, float]:
    ...
    def inspect_region(region: dict) -> None:
        ...
        if not region_id or str(region_id).lower() != "header":
            return
    ...
    x0 = min(box[0] for box in header_boxes)
    y0 = min(box[1] for box in header_boxes)
    x1 = max(box[2] for box in header_boxes)
    y1 = max(box[3] for box in header_boxes)
    return (x0, y0, x1, y1)
...
def ensure_schema(conn, embed_dim: int) -> None:
    """Ensure pgvector extension and rag_header_lines table exist."""
    ...
    with conn.cursor() as cur:
        ...
        cur.execute(
            ...
            SELECT EXISTS (
                ...
                WHERE table_schema = 'public' AND table_name = 'rag_header_lines'
            )
            """
        )
        ...
        if exists:
            cur.execute(
                ...
                SELECT atttypmod
                FROM pg_attribute
                WHERE attrelid = 'rag_header_lines'::regclass AND attname = 'embedding'
                """
            )
            row = cur.fetchone()
            if row and row[0] not in (None, -1):
                ...
                if current_dim != embed_dim:
                    raise RuntimeError(
                        "Existing rag_header_lines.embedding dimension"
        ...
        cur.execute(
            "CREATE INDEX IF NOT EXISTS rag_header_lines_doc_page_idx ON rag_header_lines(doc_id, page)"
        )
        cur.execute(
            "CREATE INDEX IF NOT EXISTS rag_header_lines_ts_idx ON rag_header_lines USING GIN(ts)"
        )
        cur.execute(
            """
            CREATE INDEX IF NOT EXISTS rag_header_lines_vec_idx
            ON rag_header_lines USING ivfflat (embedding vector_cosine_ops)
            """
        )
    conn.commit()
...
def upsert_lines(
    ...
    """Insert header lines with embeddings inside a single transaction."""
    ...
    with conn.cursor() as cur:
        for line in lines:
            ...
            cur.execute(
                """
                INSERT INTO rag_header_lines (doc_id, page, line_no, text, bbox, tokens, embedding)
...
def _fetch_line_text(
    ...
    with conn.cursor() as cur:
        cur.execute(
            "SELECT text FROM rag_header_lines WHERE doc_id = %s AND page = %s AND line_no = %s",
            (doc_id, page, line_no),
        )
        row = cur.fetchone()
    if row and row[0]:
...
def _normalize_answer(field: str, text: str, field_cfg: FieldConfig) -> str:
    ...
    if field_cfg.stop_regex:
        cleaned = field_cfg.stop_regex.sub("", cleaned).strip()
    ...
    if candidate:
        ...
        if field_cfg.stop_regex:
            candidate = field_cfg.stop_regex.sub("", candidate).strip()
        ...
        if field_cfg.require_digit and not any(ch.isdigit() for ch in candidate):
            ...
            if not any(ch.isdigit() for ch in candidate):
                candidate = cleaned
        if candidate:
            return _normalize_date_value(field, candidate.strip())
    return _normalize_date_value(field, cleaned.strip())
...
def run_queries(
    ...
    for field, synonyms in config.query_synonyms.items():
        ...
        try:
            ...
            continue
        dense_rows = dense_candidates(conn, doc_id, page, qvec, topk)
        if echo:
        ...
        keyword_raw = keyword_scores(conn, doc_id, page, query_text, list(dense_raw.keys()))
        ...
        field_cfg = config.fields.get(field, FieldConfig(name=field))
        ...
        for cid in candidate_order:
            ...
            if isinstance(line_no, int):
                for offset in (1, 2):
                    neighbor_text = _fetch_line_text(conn, doc_id, page, line_no + offset, line_cache)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Header-only RAG pipeline")
    ...
    parser.add_argument("--reindex", action="store_true", help="Reinsert header lines for doc/page")
    parser.add_argument(
        "--config",
        default="auto",
    ...
    try:
        ...
        header_bbox = find_header_bbox(s03, page=1)
        s02 = load_json(args.s02)
        tokens = extract_tokens_page1(s02)
        lines = rebuild_lines(tokens)
        header_lines = filter_lines_in_bbox(lines, header_bbox)
        if not header_lines:
            logger.warning("No header lines detected within header bbox; continuing with empty index.")
        ...
        with conn.cursor() as cur:
            cur.execute(
                "SELECT COUNT(*) FROM rag_header_lines WHERE doc_id = %s AND page = %s",
                (args.doc_id, 1),
            )
        ...
        if args.reindex and existing_count:
            with conn.cursor() as cur:
                cur.execute(
                    "DELETE FROM rag_header_lines WHERE doc_id = %s AND page = %s",
                    (args.doc_id, 1),
                )
            conn.commit()
            existing_count = 0
            logger.info("Existing rows for doc %s page 1 removed for reindex.", args.doc_id)

        if header_lines and (args.reindex or not existing_count):
            inserted = upsert_lines(conn, args.doc_id, 1, header_lines, embed_text_ollama)
            if args.echo:
                logger.info("Inserted %s header lines for doc %s page 1.", inserted, args.doc_id)
        elif args.echo:
            logger.info("Skipping indexing; existing rows already present for doc %s page 1.", args.doc_id)

        answers = run_queries(conn, args.doc_id, 1, rag_config, echo=args.echo)
        ...
        if args.echo:
            logger.info("Final answers: %s", answers)
