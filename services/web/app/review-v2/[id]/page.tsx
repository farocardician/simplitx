...
export default function ReviewPage() {
  ...
  const [rounding, setRounding] = useState<RoundingConfig>(DEFAULT_ROUNDING);
  ...
  const roundNumber = useCallback((value: number, rule: RoundingRule) => {
    if (!Number.isFinite(value)) return value;
    const factor = Math.pow(10, rule.scale);
    return Math.round((value + Number.EPSILON) * factor) / factor;
  }, []);

  const computeDerivedTaxBase = useCallback((qty: number, unitPrice: number) => {
    const qtyRounded = roundNumber(qty, rounding.qty);
    const unitPriceRounded = roundNumber(unitPrice, rounding.unit_price);
    const taxBase = roundNumber(qtyRounded * unitPriceRounded, rounding.tax_base);
    return { qtyRounded, unitPriceRounded, taxBase };
  }, [roundNumber, rounding]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const invoicePayload: InvoiceData & { rounding?: RoundingConfig } = await invoiceResponse.json();
        const uomsPayload = uomResponse.ok ? await uomResponse.json() : [];
        const roundingFromApi: RoundingConfig = invoicePayload.rounding || DEFAULT_ROUNDING;

        const normalizedItems = (invoicePayload.items || []).map((item: LineItemBase, index: number) =>
          createLineItemState(item, index)
        );

        orderKeyRef.current = normalizedItems.length;

        setRounding(roundingFromApi);
        setInvoiceData(invoicePayload);
        setInvoiceDate(invoicePayload.invoice_date);
        setInvoiceNo(invoicePayload.invoice_number);
  ...
  const validateItem = (item: LineItem, index: number): ItemErrors => {
    ...
    if (!item.description || item.description.trim() === '') {
      errors.description = {
        ...
        detail: 'Enter a description so the line item can be classified correctly.'
      };
    }
    ...
    if (item.qty === null || item.qty === undefined || isNaN(item.qty) || item.qty < 0) {
      errors.qty = {
        ...
        detail: 'Use a non-negative number. Decimals are allowed.'
      };
    }
    ...
    if (!item.hs_code || !/^\d+$/.test(item.hs_code)) {
      errors.hs_code = {
        ...
        detail: 'Only digits are allowed. The code will be validated against HS code master data.'
      };
    }
    ...
    if (!item.uom || item.uom.trim() === '') {
      errors.uom = {
        ...
        detail: 'Pick a valid unit of measure from the dropdown.'
      };
    }
    ...
    if (!item.type || (item.type !== 'Barang' && item.type !== 'Jasa')) {
      errors.type = {
        ...
        detail: 'Choose Barang (A) or Jasa (B) so HS validation can run.'
      };
    }

    // Tax base consistency: must match qty x unit_price after rounding rules
    const derived = computeDerivedTaxBase(item.qty, item.unit_price);
    const diff = Math.abs((item.amount ?? 0) - derived.taxBase);
    if (Number.isFinite(diff) && diff > 0.000001) {
      errors.tax_base = {
        message: 'Tax base must equal qty × unit price after rounding',
        detail: 'Adjust qty or unit price so the rounded multiplication matches the tax base. Current values produce a different total.'
      };
    }
  ...
  }, [initialSnapshot, items, computeDerivedTaxBase]);
  ...
  const updateItem = (index: number, field: keyof LineItem, value: any) => {
    setItems(prev => {
      ...
      if (field === 'qty' || field === 'unit_price') {
        ...
        const derived = computeDerivedTaxBase(Number(qty), Number(unitPrice));
        nextItem.qty = derived.qtyRounded;
        nextItem.unit_price = derived.unitPriceRounded;
        nextItem.amount = derived.taxBase;
    ...
    if (field === 'qty' || field === 'unit_price') {
      ...
      const derived = computeDerivedTaxBase(Number(qty), Number(unitPrice));
      newItem.qty = derived.qtyRounded;
      newItem.unit_price = derived.unitPriceRounded;
      newItem.amount = derived.taxBase;
  ...
  const handleAddItem = () => {
    ...
    const baseItem: LineItemBase = {
      ...
      qty: 1,
      unit_price: 0,
      amount: computeDerivedTaxBase(1, 0).taxBase,
      hs_code: '',
      uom: '',
      type: 'Barang',
      taxRate: 12
  ...
  const handleMerge = (anchorId: string) => {
    ...
    setItems(prev => {
      ...
      const totalQtyRaw = selectedItems.reduce((sum, current) => sum + current.qty, 0);
      const mergedQtyPre = roundNumber(totalQtyRaw, rounding.qty);
      const mergedAmountPre = roundNumber(totalAmount, rounding.tax_base);
      const mergedUnitPricePre = mergedQtyPre > 0 ? roundNumber(mergedAmountPre / mergedQtyPre, rounding.unit_price) : 0;
      const mergedDerived = computeDerivedTaxBase(mergedQtyPre, mergedUnitPricePre);
      const mergedQty = mergedDerived.qtyRounded;
      const mergedAmount = mergedDerived.taxBase;
      const mergedUnitPrice = mergedDerived.unitPriceRounded;
  ...
  const getErrorMessage = (err?: FieldError) => err?.message || '';
  const getErrorDetail = (err?: FieldError) => err?.detail || err?.message || '';

  const totalAmount = items.reduce((sum, item) => sum + (Number.isFinite(item.amount) ? item.amount : 0), 0);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="sticky top-0 z-10 bg-white border-b shadow-sm">
        <div className="max-w-7xl mx-auto px-6 py-3">
          ...
          {/* Row 2 - Seller and Total */}
          <div className="flex items-center justify-between gap-3 pt-2 border-t">
            <div className="flex items-center gap-2 text-sm text-gray-700">
              <span className="font-medium text-gray-500">Seller</span>
              <span className="text-gray-900">{invoiceData.seller_name}</span>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-6">
            ...
            <div className="flex items-start justify-between">
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1">
                    <h3 className="text-xs font-medium text-red-800">Failed to save</h3>
        ...
        <div className="space-y-2 mb-6">
          {items.map((item, index) => {
            ...
            const derivedTotals = computeDerivedTaxBase(qtyValue, unitPriceValue);
            const lineTotal = Number.isFinite(item.amount) ? item.amount : derivedTotals.taxBase;
            const taxRateValue = Number.isFinite(item.taxRate) ? item.taxRate : 12;
            const otherTaxBase = roundNumber((11 / 12) * lineTotal, rounding.other_tax_base);
            const taxAmount = roundNumber(otherTaxBase * (taxRateValue / 100), rounding.vat);
                  ...
                  <div className="flex flex-col gap-2 px-2 py-2 pl-14 text-xs text-gray-700 sm:pl-16 md:text-sm">
                      ...
                      <div className="flex flex-col gap-1.5 relative">
                        ...
                        {itemErrors.type && (
                          <div className="hidden group-hover:block absolute top-full left-0 mt-1 z-50 w-max max-w-[250px] bg-red-50 border border-red-200 rounded px-2 py-1 text-[11px] text-red-700 shadow-md">
                            {getErrorDetail(itemErrors.type)}
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-1 group relative">
                      <span className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 flex items-center gap-1">
                        ...
                        {itemErrors.description && (
                          <span className="text-red-500" title={getErrorDetail(itemErrors.description) || 'Error'}>⚠️</span>
                        )}
                      </span>
                      <div
                      ...
                      >
                        ...
                        {itemErrors.description && (
                          <div className="hidden group-hover:block absolute top-full left-0 mt-1 z-50 w-max max-w-[250px] bg-red-50 border border-red-200 rounded px-2 py-1 text-[11px] text-red-700 shadow-md">
                            {getErrorDetail(itemErrors.description)}
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-1 group relative">
                      <span className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 flex items-center gap-1">
                        ...
                        {itemErrors.hs_code && (
                          <span className="text-red-500" title={getErrorDetail(itemErrors.hs_code) || 'Error'}>⚠️</span>
                        )}
                      ...
                      >
                        ...
                        {itemErrors.hs_code && (
                          <div className="hidden group-hover:block absolute top-full left-0 mt-1 z-50 w-max max-w-[250px] bg-red-50 border border-red-200 rounded px-2 py-1 text-[11px] text-red-700 shadow-md">
                            {getErrorDetail(itemErrors.hs_code)}
                          </div>
                        )}
                    ...
                    <div className="flex flex-col gap-1 group relative">
                      <span className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 flex items-center gap-1">
                        ...
                        {itemErrors.qty && (
                          <span className="text-red-500" title={getErrorDetail(itemErrors.qty) || 'Error'}>⚠️</span>
                        )}
                      </span>
                      <div className="relative">
                        <input
                    ...
                    <div className="flex flex-col gap-1 group relative">
                      <span className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 flex items-center gap-1">
                        ...
                        {itemErrors.uom && (
                          <span className="text-red-500" title={getErrorDetail(itemErrors.uom) || 'Error'}>⚠️</span>
                        )}
                      </span>
                      <div className="relative">
                        <select
                    ...
                    <div className="flex flex-col gap-1 group relative">
                      <span className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 flex items-center gap-1">
                        ...
                        {itemErrors.unit_price && (
                          <span className="text-red-500" title={getErrorDetail(itemErrors.unit_price) || 'Error'}>⚠️</span>
                        )}
                      </span>
                      <div className="relative">
                        <input
                        ...
                        {itemErrors.unit_price && (
                          <div className="hidden group-hover:block absolute top-full left-0 mt-1 z-50 w-max max-w-[250px] bg-red-50 border border-red-200 rounded px-2 py-1 text-[11px] text-red-700 shadow-md">
                            {getErrorDetail(itemErrors.unit_price)}
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="group flex flex-col gap-1 relative">
                      <span className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 flex items-center gap-1">
                        Total
                        {itemErrors.tax_base && (
                          <span className="text-red-500" title={getErrorDetail(itemErrors.tax_base) || 'Error'}>⚠️</span>
                        )}
                      </span>
                      <div
                      ...
                      {itemErrors.tax_base && (
                        <div className="hidden group-hover:block absolute top-full left-0 mt-1 z-50 w-max max-w-[260px] bg-red-50 border border-red-200 rounded px-2 py-1 text-[11px] text-red-700 shadow-md">
                          {getErrorDetail(itemErrors.tax_base)}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <input
                        ...
                        readOnly
                        disabled
                        className="w-full h-[34px] rounded border border-gray-200 bg-gray-50 px-2 py-1 text-right tabular-nums text-gray-700"
                      />
                    </div>
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <div
                        ...
                        title={formatCurrency(otherTaxBase)}
                      >
                        {formatCurrency(otherTaxBase)}
                      </div>
                    </div>
