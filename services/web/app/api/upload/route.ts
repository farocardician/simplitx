...
async function uploadHandler(req: NextRequest, { sessionId }: { sessionId: string }) {
  try {
    ...
    try {
      ...
      return NextResponse.json(
        { error: { code: 'DB_ERROR', message: 'Failed to check for duplicate files' } },
        { status: 500 }
      );
    }
    ...
    try {
      ...
      return NextResponse.json(
        { error: { code: 'DB_ERROR', message: 'Failed to create upload job' } },
        { status: 500 }
      );
    }
    ...
    try {
      ...
      console.error('File system error creating uploads directory:', error);
      return NextResponse.json(
        { error: { code: 'FS_ERROR', message: 'Failed to create uploads directory' } },
        { status: 500 }
      );
    }
    ...
    try {
      ...
      console.error('Error saving file or updating job:', error);
      return NextResponse.json(
        { error: { code: 'FS_ERROR', message: 'Failed to save uploaded file' } },
        { status: 500 }
      );
    }
  } catch (error) {
    ...
    return NextResponse.json(
      { error: { code: 'INTERNAL_ERROR', message: 'An unexpected error occurred during upload' } },
      { status: 500 }
    );
  }
}
